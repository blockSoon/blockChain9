...
// 5. 서버는 요청을 받아서 그입장권이 유효한지 확인하고 유효한 입장권이면 요청을 처리하고
//    요청에 대한 응답을 해준다.
// 6. 입장권이 정상적인지 확인하고 썩었는지 변조가 되었는지 (변조가 되었고 썩었으면 다시 재로그인 시킨다.)
// (입장권 새로 산다.)

// Refresh token을 같이 사용하면
// Access token만 사용하면 인증 보안이 취약할수 있어서 다른사람이 Access token을 탈취했을때
// 토큰의 유효기간이 끝날때 까지는 막을수가 없다.. 그래서 유효기간을 짧게 주고 짧게 주면 
// 로그인을 계속해야하는 번거로움이 생기고(사용자가 이용하기 힘들다.) Refresh token의 유효기간을 길게주고
// Access token의 유효기간을 짧게 주어서 너무 어려운 개념은 아니고 JWT토큰을 그냥 2개 사용하는것.
// Refresh token은 유효기간을 길게 주고 Access token이 유효기간이 끝났을때 발급해주는 역활만 하면 된다.
